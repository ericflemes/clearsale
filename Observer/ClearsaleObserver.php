...
class ClearsaleObserver implements ObserverInterface
{
    ...
    public function sendPendingOrders()
    {
        try {
            ...
            if ($orders) {
                foreach ($orders as $order) {
                    try
                    {
                        ...
                        $daysAgolimit = preg_replace('/\s+/', '', $this->clearSaleTotalConfig->getValue('clearsale_configuration/cs_optional_config/days_ago_limit', \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $storeId));
                        $limitDate = $this->timezone->date()->modify('-' . $daysAgolimit . ' days');
                        $orderDate = new \DateTime($order->getCreatedAt());

                        $logMessage = 'Order ID: ' . $order->getRealOrderId() . ' | Order Status: ' . $orderStatus . ' | Order Date: ' . $orderDate->format('Y-m-d H:i:s') . ' | Process Limit Date: ' . $limitDate->format('Y-m-d H:i:s');
                        ...
                        if ($isActive && in_array($orderStatus, $pendingClearSaleStatuses) && !$this->orderExists($order) && in_array($payment->getMethodInstance()->getCode(), $creditcardMethods) && $orderDate > $limitDate) {
                            $orderId = $order->getId();
            ...
            $this->logger->error('Method sendPendingOrders error: ' .$e->getMessage());
        }
        finally {
            $this->logger->info('Method sendPendingOrders ended.');
        }
    }
    ...
    public function toClearsaleOrderObject($order, $isReanalysis, $location)
    {
        try {
            ...
            $needle = $this->clearSaleTotalConfig->getValue('clearsale_configuration/cs_optional_config/match_in_comments', \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $storeId);
            $this->logger->info('String to Match = ' . $needle);
            if(!is_null($needle) && strcmp("Type text to match here...", $needle) != 0){
                $finalmatch = '';
                $orderComment = [];

                try {
                    foreach ($order->getStatusHistoryCollection() as $status) {
			            $this->logger->info('StatusHistoryCOllection StatusObject GetComment: '. $status->getComment());
			            if (($status->getComment() != null) && (strpos($status->getComment(), $needle, 0) !== false)) {
                    ...
                    if(!empty($orderComment)){
                        ...
                        $clearsaleOrder->CustomFields[$customFieldIndex]->Value = $finalmatch;
                        $customFieldIndex = $customFieldIndex + 1;
					}
				}
				catch (\Exception $e) {
            ...
            if ($billingState) {
                $clearsaleOrder->BillingData->Address->State = $billingState;
            } else {
                $clearsaleOrder->BillingData->Address->State = "**";
            }

            $zipcodeBilling = $billingAddress->getPostcode();
            ...
            $zipcodeShipping = $shippingAddress->getPostcode();
    ...
    {
        try {
            $this->logger->info('Method getClearsaleOrderStatus called...');
            ...
            if ($orders) {
                foreach ($orders as $order) {
                    ...
                    if ($isActive && $orderStatus == $analyzing_clearsale && $orderDate > $limitDate) {
                        $environment = $this->clearSaleTotalConfig->getValue('clearsale_configuration/cs_config/environment', \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $storeId);
            ...
            $this->logger->error('Method getClearsaleOrderStatus error: ' .$e->getMessage());
        }
        finally {
            $this->logger->info('Method getClearsaleOrderStatus ended.');
        }
    }
